...
if (isset($_POST["email"]) && isset($_POST["password"])) {
    ...
    if (empty($email)) {
        header("Location: giris-yap.php?error=Mail boş bırakılmamalıdır...");
        exit();
    } else if (empty($password)) {
        header("Location: giris-yap.php?error=Şifre boş bırakılmamalıdır...");
        exit();
    } else {
        ...
        if (mysqli_num_rows($result) > 0) {
            ...
            if ($row["verified"] == "0") {
                header("Location: giris-yap.php?error=Hesabınız doğrulanmamış! Mail adresinize bakınız...");
                exit();
            }
            ...
            if ($row["email"] == $email && $row["password"] == $password) {
                ...
                header("Location: giris-yap.php?error=Mail veya şifre yanlış!");
                exit();
            }
        } else {
            header("Location: giris-yap.php?error=Mail veya şifre yanlış!");
...
<body>
    <form action="./giris-yap.php" method="POST">
        ...
        <p><a href="./kayıt-ol.php">Hesap Oluştur</a></p>
    </form>
</body>
